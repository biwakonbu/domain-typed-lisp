...
| `examples/access_control_split_schema.dtl` / `examples/access_control_split_policy.dtl` | スキーマとポリシー分割 | `cargo run -- check examples/access_control_import_entry.dtl --format json` |
...
| `examples/complex_policy_schema.dtl` / `examples/complex_policy_rules.dtl` | 複数 `@context` + split 管理 | `cargo run -- doc examples/complex_policy_import_entry.dtl --out out_complex_doc --format json` |
